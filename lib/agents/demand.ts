...
const SYSTEM_PROMPT = `You are a market intelligence analyst who follows Samuelson's Revealed Preference Theory: you trust what people DO over what they SAY. You use netnographic methods (Kozinets, 2002) to extract intelligence from online communities. You frame demand through Christensen's Jobs-to-be-Done.
...
Consult the advisor when (a) contradictory sentiment signals appear, or (b) you must infer JTBD from sparse data.
...
OUTPUT SCHEMA (return exactly this JSON, no extra keys):
{
  ...
  "job_to_be_done": {"functional": string, "social": string, "emotional": string},
  "social_evidence": [{"source": string, "finding": string, "sentiment": "positive" | "negative" | "mixed", "volume": string}],
...
export async function runDemandAgent(businessInput: string): Promise<DemandResult> {
  return runAgent<DemandResult>({
    id: "demand",
    name: "Demand Signal Analyst",
    prompt: {
      systemPrompt: SYSTEM_PROMPT,
      userMessage: `BUSINESS DESCRIPTION:\n${businessInput}\n\nRun the demand-signal analysis now. Return ONLY the JSON object.`,
